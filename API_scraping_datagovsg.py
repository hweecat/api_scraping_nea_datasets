...
1. Realtime Weather Readings across Singapore
    ...
    c. Relative Humidity across Singapore
    d. Wind Direction across Singapore
    e. Wind Speed across Singapore
...
def get_realtime_data_from_date(date, data_type):
    url = f"https://api.data.gov.sg/v1/environment/{data_type}?date={date}" # for daily API request
...
def get_data_from_date_range(date_range, data_type):
    ...
    for date in date_range:
        try:
            df_date = get_realtime_data_from_date(date, data_type)
        except ValueError:
            continue
...
def get_device_id(date, data_type):
    url = (
        "https://api.data.gov.sg/v1/environment/" + data_type + "?date=" + str(date)
    )  # for daily API request
    JSONContent = requests.get(url).json()
    content = json.dumps(JSONContent, indent=4, sort_keys=True)
    json_device_id = (
        content[content.find("stations") + 10 : -3].replace(" ", "").replace("\n", "")
    )
    ...
    for station_id in list(df_device_id["id"]):
        if station_id not in list(df_to_append["station_id"]):
            df_to_append = df_to_append.append(
                pd.DataFrame({"station_id": [station_id], "value": [np.nan]})
            )
    df_to_append_null_filled = (
        df_to_append.reset_index().drop(columns=["index"]).reset_index(drop=True)
    )
...
try:
    ...
    1. air temperature 2. rainfall 3. relative humidity 4. wind direction 5. wind speed: "
    )
    ...
    while datatype_choice:
        if datatype_choice == 1:
            ...
            datatype_entry = input(
                "Invalid input. Please choose type of data to extract from API - \
    1. air temperature 2. rainfall 3. relative humidity 4. wind direction 5. wind speed: "
            )
            datatype_choice = int(datatype_entry)
    ...
    while True:
        ...
        if stationid_choice == None:
            stationid_entry = input(
                "Invalid station ID. Please choose station ID to extract data from: "
            )
            stationid_choice = str(stationid_entry)
        else:
            break
    ...
    # create list of dataframes containing extracted reading values converted from JSON format
    df_reading = [
        df_data_from_json_to_df(reading) for reading in trange(len(df_data["readings"]))
    ]
    ...
    df_extracted_cleaned = pd.concat([df_data, df_extracted_stationid], axis=1).drop(
        columns=["readings"]
    )

    # Convert from UTC Time to SGT Time
    if (
        int((pd.__version__).split(".")[0]) == 0
        and not int((pd.__version__).split(".")[1]) >= 25
    ):
